-- ============================================================================
-- SolarFlow PM · the CRM, in one paste
-- ============================================================================
-- Everything this database is missing: contacts, deals, the contact intake
-- fields and Create Contact. Four migrations in the order they must run, in one
-- file, because the file before them (003300, which adds a value to a type) is
-- already applied here — and that boundary was the only reason they were ever
-- separate pastes.
--
-- Paste the whole thing and run it once. Safe to run again: every statement
-- skips work already done, so "already exists" errors cannot happen. Lines
-- saying "does not exist, skipping" are normal, and there will be a lot of them.
--
-- The last statement prints what is now in the database. If it says ok on every
-- column, the application will work as soon as the page is reloaded. Send that
-- result back if it does not.
-- ============================================================================

-- A table of no consequence, created first so the application can confirm this
-- paste reached the database it reads. /api/health reports whether it is there.
create table if not exists public.sf_branch_marker (
...
-- ============================================================================
-- What landed. Every column should read ok.
-- ============================================================================
select
  case when to_regclass('public.deals') is not null then 'ok' else 'MISSING' end
    as contacts_and_deals,
  case when to_regclass('public.client_channels') is not null then 'ok' else 'MISSING' end
    as channels,
  case when to_regprocedure('public.convert_deal_to_project(uuid,public.project_stage)') is not null
       then 'ok' else 'MISSING' end
    as won_deals_become_projects,
  case when exists (select 1 from information_schema.columns
                     where table_schema = 'public' and table_name = 'clients'
                       and column_name = 'mailing_street')
       then 'ok' else 'MISSING' end
    as contact_intake_fields,
  case when to_regprocedure('public.create_contact(jsonb,jsonb)') is not null
       then 'ok' else 'MISSING' end
    as create_contact,
  (select count(*) from public.deals)  as deals_now,
  (select count(*) from public.clients) as contacts_now;
