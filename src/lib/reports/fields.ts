...
export const REPORT_FIELDS: ReportField[] = [
  ...
  date('s1.survey_scheduled', 'Site Survey Scheduled Date', 'survey', 's1.survey_scheduled_date', { needs: ['s1'] }),
  status('s1.roof_type', 'Roof Type', 'survey', 's1.roof_type', { needs: ['s1'] }),
  num('s1.roof_age', 'Roof Age (years)', 'survey', 's1.roof_age_years', { needs: ['s1'] }),
  status('s1.roof_condition', 'Roof Condition', 'survey', 's1.roof_condition', { needs: ['s1'] }),
  num('s1.stories', 'Stories', 'survey', 's1.stories', { needs: ['s1'] }),
  num('s1.main_panel_amps', 'Main Panel Rating (A)', 'survey', 's1.main_panel_rating_amps', { needs: ['s1'] }),
  num('s1.bus_bar_amps', 'Bus Bar Rating (A)', 'survey', 's1.bus_bar_rating_amps', { needs: ['s1'] }),
  num('s1.main_breaker_amps', 'Main Breaker (A)', 'survey', 's1.main_breaker_amps', { needs: ['s1'] }),
  status('s1.panel_upgrade', 'Main Panel Upgrade Needed', 'survey', 's1.panel_upgrade_needed', { needs: ['s1'] }),
  text('s1.meter_number', 'Meter Number', 'survey', 's1.meter_number', { needs: ['s1'], groupable: false }),
  text('s1.utility_account', 'Utility Account Number', 'survey', 's1.utility_account_number', { needs: ['s1'], groupable: false }),
  status('s1.attic_access', 'Attic Access', 'survey', 's1.attic_access', { needs: ['s1'] }),
  num('s1.trenching_ft', 'Trenching Distance (ft)', 'survey', 's1.trenching_distance_ft', { needs: ['s1'] }),
  ...
  num('s2.final_kw', 'Final System Size (kW)', 'design', 's2.final_system_size_kw', { needs: ['s2'] }),
  num('s2.final_modules', 'Final Module Count', 'design', 's2.final_module_count', { needs: ['s2'] }),
  num('s2.production_kwh', 'Year-1 Production Estimate (kWh)', 'design', 's2.production_estimate_kwh', { needs: ['s2'] }),
  num('s2.offset_percent', 'Usage Offset (%)', 'design', 's2.offset_percent', { needs: ['s2'] }),
  num('s2.revision', 'Design Revision', 'design', 's2.design_revision', { needs: ['s2'] }),
  date('s2.customer_approved', 'Customer Approved Design Date', 'design', 's2.customer_approval_date', { needs: ['s2'] }),
  text('s2.engineering_firm', 'Engineering Firm', 'design', 's2.engineering_firm', { needs: ['s2'] }),
  status('s2.pe_stamp_required', 'PE Stamp Required', 'design', 's2.pe_stamp_required', { needs: ['s2'] }),
  ...
  text('s3.permit_number', 'Permit Number', 'permit', 's3.permit_number', { needs: ['s3'], groupable: false }),
  date('s3.permit_expiry', 'Permit Expiry Date', 'permit', 's3.permit_expiry_date', { needs: ['s3'] }),
  money('s3.permit_fee', 'Permit Fee', 'permit', 's3.permit_fee', { needs: ['s3'], financial: true }),
  status('s3.permit_method', 'Permit Submitted Via', 'permit', 's3.permit_submission_method', { needs: ['s3'] }),
  text('s3.ica_number', 'ICA Application Number', 'permit', 's3.ica_application_number', { needs: ['s3'], groupable: false }),
  status('s3.meter_swap', 'Meter Swap Required', 'permit', 's3.meter_swap_required', { needs: ['s3'] }),
  text('s3.hoa_name', 'HOA Name', 'permit', 's3.hoa_name', { needs: ['s3'] }),
  ...
  text('s4.vendor', 'Vendor / Distributor', 'procurement', 's4.vendor_name', { needs: ['s4'] }),
  text('s4.po_number', 'PO Number', 'procurement', 's4.po_number', { needs: ['s4'], groupable: false }),
  date('s4.order_date', 'Order Date', 'procurement', 's4.order_date', { needs: ['s4'] }),
  date('s4.expected_delivery', 'Expected Delivery Date', 'procurement', 's4.expected_delivery_date', { needs: ['s4'] }),
  status('s4.location', 'Material Location', 'procurement', 's4.material_location', { needs: ['s4'] }),
  money('s4.material_cost', 'Material Cost', 'procurement', 's4.material_cost', { needs: ['s4'], financial: true }),
  num('s4.delivery_lag', 'Delivery vs Expected (days)', 'procurement', daysBetween('s4.expected_delivery_date', 's4.material_delivered_date'), { needs: ['s4'] }),
  // Key kept from when this was "Drive Updated", so saved reports keep working.
  ...
  text('s5.crew_lead', 'Crew Lead', 'install', 's5.crew_lead', { needs: ['s5'] }),
  num('s5.crew_size', 'Crew Size', 'install', 's5.crew_size', { needs: ['s5'] }),
  num('s5.duration_days', 'Install Duration (days)', 'install', 's5.install_duration_days', { needs: ['s5'] }),
  date('s5.mpu_completed', 'Main Panel Upgrade Completed', 'install', 's5.mpu_completed_date', { needs: ['s5'] }),
  date('s5.homeowner_signoff', 'Homeowner Sign-off Date', 'install', 's5.homeowner_signoff_date', { needs: ['s5'] }),
  ...
  date('s6.inspection_scheduled', 'Inspection Scheduled Date', 'inspection', 's6.inspection_scheduled_date', { needs: ['s6'] }),
  text('s6.inspector', 'Inspector', 'inspection', 's6.inspector_name', { needs: ['s6'] }),
  date('s6.reinspection', 'Re-inspection Date', 'inspection', 's6.reinspection_date', { needs: ['s6'] }),
  text('s6.pto_number', 'PTO Application Number', 'inspection', 's6.pto_application_number', { needs: ['s6'], groupable: false }),
  date('s6.meter_set', 'Net Meter Set Date', 'inspection', 's6.meter_set_date', { needs: ['s6'] }),
  status('s6.monitoring_platform', 'Monitoring Platform', 'inspection', 's6.monitoring_platform', { needs: ['s6'] }),
  ...
  date('s7.warranty_registered', 'Warranties Registered Date', 'complete', 's7.warranty_registration_date', { needs: ['s7'] }),
  date('s7.final_payment', 'Final Payment Received Date', 'complete', 's7.final_payment_received_date', { needs: ['s7'] }),
  date('s7.closeout_sent', 'Close-out Packet Sent Date', 'complete', 's7.closeout_packet_sent_date', { needs: ['s7'] }),
  date('s7.review_requested', 'Review Requested Date', 'complete', 's7.review_requested_date', { needs: ['s7'] }),
  status('s7.referral_asked', 'Referral Asked', 'complete', 's7.referral_asked', { needs: ['s7'] }),
