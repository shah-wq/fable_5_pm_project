...
- **Module 4 — Stage forms**: the six data-entry forms from the Stage Field Specification — payment milestones (Down Payment, Cash M1–M3), the five Permit tracks (Permit · ICA · HOA · Cash M2 · HDM NTP), partner-labelled Finance M1/M2, collapsible track cards with status chips, live Days counters (amber past 14 days), status→date auto-stamping, conditional requirements, file uploads (shading report, install pictures), and the Drive Updated gate closing every stage. One registry (`src/lib/stages/fields.ts`) drives the renderer, the persistence allowlist, and the advance gates.
