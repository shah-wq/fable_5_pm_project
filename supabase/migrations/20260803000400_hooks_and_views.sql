...
-- -----------------------------------------------------------------------------
-- Custom access token hook: stamp the profile's role into the JWT as
-- `user_role` so RLS never has to hit profiles on the hot path.
-- Enabled in config.toml ([auth.hook.custom_access_token]); on hosted
-- projects also enable it under Authentication → Hooks.
-- -----------------------------------------------------------------------------
