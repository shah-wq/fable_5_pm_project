...
text "$W/page.html" | grep -q "13 migrations are missing" || fail "the screen does not say thirteen are missing"
...
assert j['behind'] == ['20260803003400_crm_foundation.sql', '20260803003500_deals.sql',
                       ...
                       '20260803004600_stage_fields_solar.sql'], j['behind']
assert '20260803004600_stage_fields_solar.sql' in j['bundled'], 'the deployment does not carry its own files'
print('STATE-OK')
PY
...
assert files == ['20260803003400_crm_foundation.sql', '20260803003500_deals.sql',
                 ...
                 '20260803004400_esignature.sql',
                 '20260803004500_sales_see_dealer_names.sql',
                 '20260803004600_stage_fields_solar.sql'], files
...
[ "$(q "select count(*) from public.schema_migrations where name >= '20260803003400'")" = 13 ] \
